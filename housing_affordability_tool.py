...
num_units = st.slider("How many units would you like to compare?", 1, 10, 1)
...
for i in range(num_units):
    ...
    square_feet = st.number_input(f"Square footage for Unit {i + 1}", min_value=1, max_value=10000, key=f"sf_{i}")
    est_bedrooms = max(1, min(round((square_feet * 0.28) / 200), 5))
    st.text(f"Assuming this unit has approximately {est_bedrooms} bedrooms based on square footage.")
...
selected_regions = st.multiselect("Which locations are you interested in with regard to AMI?", valid_regions, default=valid_regions)

valid_ami_values = [30] + list(range(50, 155, 5))
num_amis = st.slider("How many AMI values would you like to show on the chart?", 1, 10, 1)
amis = []
